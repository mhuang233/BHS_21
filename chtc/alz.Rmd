...
```
## in case they want blackand white requirement
```{r}
ws_sigma0 %>% ggplot() +
...
#round_kld0 <- mean_kld0 %>% 
#  mutate(kl_mean = round(kld_mean, 6)) %>% select(!kld_mean)

#round_kld5 <- mean_kld5 %>% 
#  mutate(kl_mean = round(kld_mean, 6)) %>% select(!kld_mean)
...
### round the mean of kld
#round_kld0 %>% kable(., caption = "sigma0") %>% kable_styling()
#round_kld5 %>% kable(., caption = "sigma5") %>% kable_styling()
```
